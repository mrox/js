...
function Player(input, enableMatching = false, camera_id)
{
    let self = this;
    this.width = 1280;
    this.height = 720;
...
Player.prototype.deleteMe = function () {
    clearInterval(this._interval);
    console.log("Delete Me");
    if (this.h264reader)
        this.h264reader.deleteMe();
}
...
Player.prototype.pushEngineInfo = function (message) {
    ...
    let it = new MixedItem();
    it.timestamp = parseInt(message.timestamp);
    let info = {
...
Player.prototype.draw = function (n) { 
    if (this.mixer.pics.length) {
        ...
        if(item_log==0){item_log = item;}
        if(item.einfo.length == 0){ item = item_log; item_log=0; }else{ item_log = item;}
...
Player.prototype.drawMatchingInfo = function (item) {
    ...
    if (item.state == TS_STATE.FULL) {
        if (item.einfo) {
            ...
            ctx.fillStyle  = config.tracking.color;
            ...
            for (c of item.einfo.speeds) {
                ...
                ctx.strokeStyle = "green";
                ctx.lineWidth = 10;
                if(c.value == 'Do') { c.value = 'Đỏ';}
                if(c.value == 'Vang') { c.value = 'Vàng';}
                if(c.value == 'TwoWheeler') { c.value = 'Bike';}
                // if(c.value == 'Car') { c.value = 'ÔTô';}
                // ctx.strokeStyle = 'black';
                if (Number.isInteger(c.value)) {
                    ctx.strokeStyle = "green";
                    ctx.strokeText(parseInt(c.value) + "km/h", c.x, c.y);
                    // ctx.textAlign = "center";
                    ctx.fillText(parseInt(c.value) + "km/h", c.x, c.y);
                } else {
                    if(c.value == 'Vuot den do'){
                        c.value = 'Vượt đèn đỏ';
                        ...
                        ctx.rect(c.x - 60,c.y - 45,260,60);
                        ctx.stroke();
                        ctx.fillText(c.value, c.x - 50, c.y);
                    }else{
                        ctx.shadowBlur=50;
                        ctx.shadowColor="red";
                        ctx.strokeStyle = "red";
                    ...
                    //ctx.strokeText(c.value, c.x, c.y);
                    // ctx.textAlign = "center";
                }
            }
            // ctx.fill();
